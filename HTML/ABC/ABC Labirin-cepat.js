...
/* ====================================================================== */
/*  - Labirin grid di-generate dengan DFS (maze klasik) + beberapa loop.  */
/*  - Artificial Bee Colony (ABC):                                        */
/*      • 1 lebah = 1 jalur start → goal.                                 */
/*      • Employed bees: local search (modifikasi jalur).                 */
/*      • Onlooker bees: pilih employed (roulette wheel) lalu modifikasi. */
/*      • Scout bees: reset kalau terlalu lama tidak membaik.             */
/*  - Jalur terbaik global ditampilkan sebagai garis tebal biru.          */
/* ====================================================================== */
  ...
  cariTetanggaDanUpdate() {
    ...
    if (jalurBaru) {
      ...
      if (fitBaru < this.fitness) {
        ...
        this.fitness = fitBaru;   // <- BUG sebelumnya: fitBarBaru
        this.trial = 0;
        perbaruiJalurTerbaik(this);
      } else {
        this.trial++;
...
function resetMazeAndColony() {
  // Ukuran grid berdasarkan ukuran kanvas sekarang
...
function setup() {
  ...
  if (!ui.menuPanel) {
    ...
    const deskripsi = createP(
      ...
      'Geser slider untuk mengubah perilaku koloni.'
    );
    ...
    buatSlider('PropEmployed', 0.1, 0.9, PROP_EMPLOYED, 0.05, ui.menuPanel, (val) => {
...
function draw() {
  ...
  // Jalankan beberapa iterasi Bee Colony per frame
  jalankanSiklusLebah(5);
